...
imgDir = fullfile('Colour Normalized Malaria Slide Images');
...
%% Convert Images and Pixel Labels
imds = convertImages(imds, imgDir);
pxds = convertPixelLabels(pxds, labelDir);
...
% The use of SegNet network layers
...
pic_num = 1;
J = readimage(imds, pic_num);
PL = readimage(pxds, pic_num);
GT = labeloverlay(J, PL, 'Colormap', cmap, 'Transparency', 0.5);
figure(3);
imshow(GT);
pixelLabelColorbar(cmap,classes);
...
S = semanticseg(J, net);
B = labeloverlay(J, S, 'Colormap', cmap, 'Transparency', 0.5);
figure(4)
